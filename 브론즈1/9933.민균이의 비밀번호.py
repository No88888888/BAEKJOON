...
N = int(input())
pw = [list(input()) for _ in range(N)] # 파일 내 단어 리스트
out = False                            # 코드 종료 위한 bool 형식 값
for i in range(N):
    if out:
        pass
    for j in range(N):
        if pw[i] == pw[i][::-1]:    #단어 자체가 회문이라면
            ans1 = len(pw[i])       # 해당 단어의 길이와
            ans2 = pw[i][ans1//2]   # 단어의 중간 문자 반환
            out = True
        if pw[i] == pw[j][::-1]:    #단어 두개가 서로 회문관계라면
            ans1 = len(pw[i])       # 둘 중 무엇이든 단어의 길이와
            ans2 = pw[i][ans1//2]   # 단어의 중간 문자 반환
            out = True
print(ans1, ans2)
