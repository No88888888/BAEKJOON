...
pocketmon = defaultdict(str)

for i in range(1, N+1):
    pocketmon[i] = input().strip()                  # 번호 : 포켓몬
pocketmon2 = {v: k for k, v in pocketmon.items()}   # 포켓몬 : 번호
for j in range(M):
    mon = input().strip()
    if mon.isalpha():                               # 포켓몬 이름이면
        print(pocketmon2[mon])                      # 번호 출력
    else:                                           # 번호면
        print(pocketmon[int(mon)])                  # 포켓몬 이름 출력
